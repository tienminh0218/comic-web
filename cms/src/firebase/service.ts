...
import { convertsData, convertData } from "utils";
...
const getDocsDb = async <T>(collectionName: string, opt?: QueryConstraint): Promise<T> => {
    const docRef = collection(dbStore, collectionName);
    const q = opt ? query(docRef, opt) : query(docRef);
